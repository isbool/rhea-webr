...
for (i in dependant_variables_start:dim(input_table)[2])
{
  ...
  test_table <- tryCatch (xtabs(my_test_vector ~ independent_variable,addNA = T,na.rm = T ),error = function(i) {xtabs(my_test_vector ~ independent_variable)})
...
prefix = paste(independant_variable_name,strsplit(input_filename,"[.]")[[1]][1],sep="_")
